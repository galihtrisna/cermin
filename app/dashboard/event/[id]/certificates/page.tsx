...
import { getAccessToken } from "@/lib/token"; // [PENTING] Import ini untuk ambil token
...
const CertificateSettingsPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchEventSettings = async () => {
      try {
        const token = getAccessToken(); // Ambil token (opsional utk GET, tapi bagus utk konsistensi)
        const event = await getEventCertificateSettings(id, token || undefined);
  ...
  const handleSave = async (e: FormEvent) => {
    ...
    try {
      ...
      const token = getAccessToken(); // [FIX] Ambil token dari LocalStorage
      if (!token) throw new Error("Sesi Anda habis. Silakan login ulang.");

      let finalUrl = existingBackground;

      // Upload file jika ada
      if (backgroundFile) {
        const uploadedUrl = await uploadCertificateBackground(backgroundFile, token); // Pass token
        if (!uploadedUrl) throw new Error("Gagal upload gambar background.");
        finalUrl = uploadedUrl;
      }

      if (!finalUrl) throw new Error("Silakan pilih gambar background terlebih dahulu.");

      // Simpan URL ke Event
      const ok = await updateEventCertificateBackground(id, finalUrl, token); // Pass token
      if (!ok) throw new Error("Gagal menyimpan pengaturan sertifikat.");
  ...
  // 3. Fungsi Kirim Sertifikat
  const handleIssueCertificates = async () => {
    if (!existingBackground) {
      alert("Harap simpan template background terlebih dahulu.");
      return;
    }
    if (!id) return;

    const confirmMsg = "Sertifikat akan dikirim ke seluruh peserta yang 'Hadir'. Lanjutkan?";
    ...
    try {
      const token = getAccessToken(); // [FIX] Ambil token
      if (!token) throw new Error("Sesi Anda habis. Silakan login ulang.");

      const res = await issueCertificates(id, token); // Pass token

      if (!res) throw new Error("Gagal mengirim sertifikat (Cek log backend).");

      setMessage({
        type: "success",
        text: `Berhasil! Email sertifikat telah dikirim ke ${res.issued_count ?? 0} peserta.`,
      });
    } catch (error) {
      console.error(error);
      setMessage({
        type: "error",
        text: error instanceof Error ? error.message : "Gagal mengirim sertifikat.",
      });
    } finally {
      setSending(false);
    }
  ...
  // ... (Sisa kode UI render tidak perlu diubah, cukup copy-paste bagian return dari file lama) ...
  // Helper functions
  const processFile = (file: File) => {
      if (!file.type.startsWith("image/")) {
        setMessage({ type: "error", text: "File harus berupa gambar (PNG/JPG)." });
        return;
      }
