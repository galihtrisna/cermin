...
import { Calendar, MapPin, Tag, Users, Loader2, Upload, X } from "lucide-react";
import { createEvent, uploadEventImage } from "@/app/actions/event"; // Pastikan import action ini ada
...
const NewEventPage = () => {
  ...
  const [isUploading, setIsUploading] = useState(false); // State loading upload gambar
  ...
  const handleFileChange = async (file: File) => {
    ...
    try {
      ...
      setBannerUrl(url); // Simpan URL yang dikembalikan backend
    } catch (err: any) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        location,
        organizer: "Self", // Nanti backend akan overwrite ini dengan user login
        image: bannerUrl, // URL gambar yang sudah diupload

        // Tambahan field yang dibutuhkan backend controller
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {!bannerUrl ? (
              <div
                ...
                className={`
                  ...
                  border-2 border-dashed border-[#E4E7F5] hover:border-[#50A3FB] 
                  rounded-3xl p-8 transition-all bg-white/50
              ...
              >
                ...
                />
                <div className="w-12 h-12 rounded-2xl bg-[#50A3FB]/10 flex items-center justify-center text-[#50A3FB] group-hover:scale-110 transition-transform">
                  {isUploading ? (
                ...
                <div className="text-center">
                  <p className="text-sm font-medium text-[#344270]">
                    {isUploading ? "Sedang mengunggah..." : "Klik atau seret gambar ke sini"}
                  </p>
                  <p className="text-xs text-[#34427099] mt-1">
                    PNG, JPG (Max 5MB)
                  </p>
                </div>
              </div>
              ...
              // Preview Gambar
              <div className="relative rounded-3xl overflow-hidden border border-[#E4E7F5] bg-slate-50 aspect-video md:aspect-[21/9]">
                <img 
                  src={bannerUrl} 
                  alt="Banner Preview" 
          ...
          <div className="pt-2">
            <button
              type="submit"
              disabled={submitting || isUploading} // Disable jika sedang submit ATAU sedang upload gambar
              className="
                w-full rounded-2xl 
                bg-[#50A3FB] 
                text-white 
